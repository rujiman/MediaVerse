...
/**
 * Modelo para un item guardado en una de las listas "Pienso ver / Pienso
 * jugar / Pienso escuchar" — cosas que llamaron la atención pero todavía
 * no se han visto/jugado/escuchado, y que el usuario no ha decidido (o no
 * quiere) marcar todavía como favoritas.
 *
 * Es una lista TOTALMENTE independiente de FavoritesService: un mismo
 * título puede estar en ambas a la vez sin conflicto, y quitarlo de una
 * no afecta a la otra.
 *
 * Comparte la misma forma que FavoriteItem (mismos campos: episodios,
 * tmdbId, trailerKey, previewUrl, anilistId, igdbId...) para poder
 * reutilizar exactamente el mismo DetailView/DetailViewController que ya
 * existe — episodios, recomendaciones, tráiler y preview funcionan igual
 * para un item de "Pienso ver" que para un favorito, sin duplicar esa
 * pantalla. La única diferencia estructural es folderId, que no existe
 * en FavoriteItem.
 */
public class PlanItem {
    ...
    private boolean viewed; // por si se empieza a ver/jugar antes de decidir pasarlo a favoritos
    ...
    // Datos adicionales para que el detalle se vea completo (igual que en FavoriteItem)
    ...
    // Carpeta a la que pertenece este item dentro de su lista, o null si
    // está suelto (sin carpeta) en la raíz de la lista.
    private String folderId;

    public PlanItem() {}

    public PlanItem(MediaItem item) {
        this.id = generateId();
    ...
    /**
     * Reconstruye un MediaItem equivalente, para reutilizar DetailView
     * (mismo patrón que FavoriteItem.toMediaItem()).
     */
